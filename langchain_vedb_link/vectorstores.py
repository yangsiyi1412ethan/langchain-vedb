...
class VeDB(VectorStore):
    ...
    # async def aembed_documents_parallel(self, texts: list[str]) -> list[list[float]]:
    #     async def aembed_one(text: str) -> list[float]:
    #         return await self.embedding.aembed_documents([text])[0]  # 假设是 batch 接口
    #
    #     return await asyncio.gather(*[aembed_one(text) for text in texts])
    ...
    #         embedding = self.embedding.embed_query(query)
    #         return [
    #             doc for doc, _, _ in self._similarity_search_with_score_by_vector(
    #                 embedding=embedding, k=k, **kwargs
    #             )
    #         ]
    #
